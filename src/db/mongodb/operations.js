 ...
 * });
 */
const find = ({ table, key = {} }) => new Promise(async (resolve, reject) => {
    try {
        ...
        delete key.options;
        delete key?.query;
        let res = await models[table.toLowerCase()][method](key, options)[noPaginate ? 'populate' : 'then'](populate?.path, populate?.select?.split(' '));
        resolve(res);
    }
    catch (e) { reject(e) }
});
 ...
 */
const findOne = async ({ table, key = {} }) => new Promise(async (resolve, reject) => {
    try {
        if (key.id) key._id = key.id; delete key.id;
        if (Object.keys(key).length < 1) resolve(null);
        const res = await models[table.toLowerCase()].findOne(key).populate(key.populate?.path, key.populate?.select?.split(' '));
        resolve(res);
    }
    catch (e) { reject(e) }
});
 ...
 * });
 */
const create = async ({ table, key }) => new Promise(async (resolve, reject) => {
    try {
        const elem = await new models[table.toLowerCase()](key);
 ...
 * });
 */
const update = async ({ table, key }) => new Promise(async (resolve, reject) => {
    try {
        if (key.id) key._id = key.id; delete key.id;
        const element = await models[table.toLowerCase()].findOne(key);
 ...
 */
const remove = async ({ table, key }) => new Promise(async (resolve, reject) => {
    try {
        if (key.id) key._id = key.id; delete key.id;
        const element = await models[table.toLowerCase()].findOne(key);
        if (!element) resolve(element);
 ...
 */
const removeAll = async ({ table }) => new Promise(async (resolve, reject) => {
    try {
        const res = await models[table.toLowerCase()].deleteMany({});
        resolve(res);
    }
    catch (e) { reject(e) }
});
...
module.exports = { find, findOne, create, remove, update, save, removeAll };